...
```{r}
ggplot(education_df_clean,aes(Country.Name, Value, color = Indicator.Name)) +
  geom_bar(stat='identity') 
```

```{r}
ggplot(education_df_clean, mapping = aes(Country.Code,Value)) +
         geom_bar(aes(fill = Indicator.Name),
                  position = "dodge")
```


```{r}
colnames(female_df)[6] <- "female_education"
colnames(male_df)[6] <- "male_education"
education = data.frame(female_df$Country.Name,female_df$female_life_expectancy,male_df$male_life_expectancy)
```
