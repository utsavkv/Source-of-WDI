...
df.gdp <- df.long %>% 
  ...
  group_by(Year) %>%
  summarise(GDP = sum(Value, na.rm = TRUE), Missing = sum(is.na(Value))*100/n()) %>%
  ungroup() 

p1 <- df.gdp %>%
  ggplot(aes(Year, GDP/MIL)) + 
  geom_line() + 
...
p2 <- df.gdp %>%
  ggplot(aes(Year, Missing)) + geom_col(fill = 'red', alpha = 0.5) + labs(y = '% of missing values') + theme_bw(8)
